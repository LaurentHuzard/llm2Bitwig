...
type DeviceRackProps = {
    devices: Device[];
    onBypass: (deviceIndex: number, currentBypassState: boolean) => void;
    onDelete: (deviceIndex: number) => void;
};
...
export function DeviceRack({ devices, onBypass, onDelete }: DeviceRackProps) {
    ...
    return (
        <div className="flex space-x-2 overflow-x-auto p-4 bg-gray-900/50 rounded-2xl border border-gray-800 backdrop-blur-sm min-h-[160px]">
            {devices.map((device, idx) => {
                const deviceIndex = device.index ?? idx;
                return (
                <DeviceCard
                    key={deviceIndex}
                    device={device}
                    onBypass={() => onBypass(deviceIndex, !device.enabled)} // device.enabled is true if NOT bypassed
                    onDelete={() => onDelete(deviceIndex)}
                />
                );
            })}
        </div>
...
function DeviceCard({ device, onBypass, onDelete }: DeviceCardProps) {
