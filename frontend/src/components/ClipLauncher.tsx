...
type ClipLauncherProps = {
    clipGrid: ClipSlotData[][];
    tracks: Track[];
    onLaunchClip: (trackIndex: number, sceneIndex: number) => void;
    onStopTrack: (trackIndex: number) => void;
    onRecordClip: (trackIndex: number, sceneIndex: number) => void;
};
...
export function ClipLauncher({ clipGrid, tracks, onLaunchClip, onStopTrack, onRecordClip }: ClipLauncherProps) {
                    ...
                    <div className="inline-flex flex-col space-y-1">
                        ...
                        {clipGrid.map((trackSlots, trackIdx) => (
                            <div key={trackIdx} className="flex space-x-1">
                                {trackSlots.map((slot, slotIdx) => {
                                    const trackIndex = slot.trackIndex ?? trackIdx;
                                    const sceneIndex = slot.sceneIndex ?? slotIdx;
                                    return (
                                    <ClipSlot
                                        key={`${trackIndex}-${sceneIndex}`}
                                        slot={{ ...slot, trackIndex, sceneIndex }}
                                        onLaunch={() => onLaunchClip(trackIndex, sceneIndex)}
                                        onRecord={() => onRecordClip(trackIndex, sceneIndex)}
                                    />
                                    );
                                })}
                            </div>
...
function ClipSlot({ slot, onLaunch, onRecord }: ClipSlotProps) {
    const trackIndex = slot.trackIndex ?? 0;
    const sceneIndex = slot.sceneIndex ?? 0;
    const hasContent = slot.hasContent ?? slot.hasClip ?? false;
    const { isPlaying, isRecording, isPlaybackQueued } = slot;
    ...
    return (
        <button
            ...
            title={`Track ${trackIndex + 1}, Scene ${sceneIndex + 1}`}
        >
            {icon}
            {!hasContent && (
