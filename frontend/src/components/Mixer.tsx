type TrackColor = { r: number; g: number; b: number };
...
type MixerProps = {
    ...
    onSetPan: (index: number, value: number) => void;
...
export function Mixer({ tracks, onSetVolume, onSetPan, selectedIndex, onSelectTrack }: MixerProps) {
    // Ensure we have 8 slots even if tracks is empty
    const slots = Array(8)
        .fill(null)
        .map((_, i) => tracks[i] || { name: `Track ${i + 1}`, volume: 0, pan: 0.5 });
...
function MixerChannel({ index, track, onVolumeChange, onPanChange, isSelected, onSelect }: MixerChannelProps) {
