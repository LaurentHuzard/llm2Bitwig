...
export class TransportModule implements ControllerModule {
  ...
  handleRequest(method: string, params?: RequestParams): unknown {
    switch (method) {
      ...
      case "transport.setTempo":
        if (params && params[0]) {
          this.transport.tempo().value().setRaw(params[0] as number);
          return "OK";
        }
      ...
      case "transport.setPosition":
        if (params && params[0]) {
          this.transport.getPosition().set(params[0] as number);
          return "OK";
        }
      ...
      case "transport.setLoopStart":
        if (params && params[0] !== undefined) {
          this.transport.getInPosition().set(params[0] as number);
          return "OK";
        }
        throw "Missing loop start parameter";
      case "transport.setLoopEnd":
        if (params && params[0] !== undefined) {
          this.transport.getOutPosition().set(params[0] as number);
          return "OK";
        }
      ...
      case "transport.time_signature":
        if (params && params[0] !== undefined && params[1] !== undefined) {
          this.transport.timeSignature().set(params[0] as number, params[1] as number);
          return "OK";
        }
      ...
      case "transport.set_punch_in":
        if (params && params[0] !== undefined) {
          this.transport.isPunchInEnabled().set(params[0] as boolean);
          return "OK";
        }
        throw "Missing punch in state parameter";
      case "transport.set_punch_out":
        if (params && params[0] !== undefined) {
          this.transport.isPunchOutEnabled().set(params[0] as boolean);
          return "OK";
        }
