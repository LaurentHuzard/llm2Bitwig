...
async function run() {
    ...
    try {
        ...
        const grid = await client.callTool<Array<Array<{ name?: string }>>>("clip_get_grid");
        const firstName = grid[0]?.[0]?.name;
        console.log("Clip Grid retrieved. First clip name:", firstName);
        if (firstName !== "Clip 0-0") throw new Error("Clip name mismatch: " + firstName);
