...
async function run() {
  ...
  try {
    ...
    const punchStatus = await client.callTool("transport_get_punch_status");
    ...
    const overdubStatus = await client.callTool("transport_get_overdub_status");
    ...
    const trackList = await client.callTool("track_list");
    assert(Array.isArray(trackList), "track_list should return array");
    
    if (trackList.length > 0) {
      console.log("Testing track_get_info...");
      const trackInfo = await client.callTool("track_get_info", { index: 0 });
    ...
    const cursorTrack = await client.callTool("cursor_track_get_status");
    ...
    const cursorDevice = await client.callTool("cursor_device_get_status");
    ...
    const cursorClip = await client.callTool("cursor_clip_get_status");
