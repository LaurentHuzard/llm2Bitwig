...
async function run() {
    ...
    try {
        ...
        const instRes = await client.callTool<string>("application_create_instrument_track");
        assert.strictEqual(instRes, "OK");

        // 2. Create Audio Track
        console.log("Testing application_create_audio_track...");
        const audioRes = await client.callTool<string>("application_create_audio_track");
